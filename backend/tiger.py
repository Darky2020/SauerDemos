...
def tiger_round(a, b, c, x, mul):
    ...
    a -= t1[(c >> (0 * 8)) & 0xFF] ^ t2[(c >> (2 * 8)) & 0xFF] ^ t3[(c >> (4 * 8)) & 0xFF] ^ t4[
        (c >> (6 * 8)) & 0xFF]
    b += t4[(c >> (1 * 8)) & 0xFF] ^ t3[(c >> (3 * 8)) & 0xFF] ^ t2[(c >> (5 * 8)) & 0xFF] ^ t1[
        (c >> (7 * 8)) & 0xFF]
    b *= mul
...
def tiger_compress(message, res):
    # setup
    a = res[0]
    b = res[1]
    c = res[2]
    ...
    for j in range(0, 8):
        ...
        tmp = struct.unpack('Q', message[j * 8:j * 8 + 8])[0]
        x.append(tmp)
...
def tiger_hash(message):
    ...
    length = len(message)
    while i < length - 63:
        tiger_compress(message[i:i + 64], res)
        i += 64
    temp = array.array('b', message[i:].encode())
    j = len(temp)
    temp.append(0x01)
    j += 1
    ...
    temp.frombytes(struct.pack('Q', length << 3))
